...
impl HSV {
        ...
        for y in 0..height {
            for x in 0..width {
                ...
                let theta: f32 = {
                    ...
                    let denominator =
                        ((red - green).powf(2.0) + (red - blue) * (green - blue)).powf(0.5);
                ...
                let intensity: f32 = { (red + blue + green) / 3.0 };
    ...
    pub fn hsv_to_rgb(hsv: (u16, f32, u8)) -> [u8; 3] {
        ...
        if (hue as u16) < 120 {
            ...
            let red = brightness
                * (1.0
                    + (saturation * hue.to_radians().cos().to_degrees())
                        / (60.0 - hue).to_radians().cos().to_degrees());
            let green = 3.0 * brightness - (red + blue);

            [
                clamp(red.round(), 0.0, 255.0) as u8,
                clamp(green.round(), 0.0, 255.0) as u8,
                clamp(blue.round(), 0.0, 255.0) as u8,
            ]
...
impl HSV {
    pub fn enumerate_hues(&self) -> impl Iterator<Item = ((usize, usize), &u16)> {
        ...
        self.hue_data.iter().enumerate().map(move |(index, hue)| {
            let x = index % width;
            let y = index / width;
            ((x, y), hue)
        })
