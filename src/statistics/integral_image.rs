...
impl IntegralImage {
    ...
    pub fn get_point(&self, x: u32, y: u32) -> u64 {
        self.container[(x + y * self.width) as usize]
    }

    pub fn mut_point(&mut self, x: u32, y: u32, new_value: u64) {
        self.container[(x + y * self.width) as usize] = new_value;
    }

    pub fn region_sum(&self, region: Rectangle) -> u64 {
...
pub fn integral_image(image: &GrayImage) -> (IntegralImage, IntegralImage) {
    ...
    for y in 1..height {
        for x in 1..width {
            ...
            let current_pixel = u64::from(image.get_pixel(x - 1, y - 1)[0]);
