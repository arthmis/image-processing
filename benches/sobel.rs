...
pub fn sobel_x_y(c: &mut Criterion) {
    ...
    group.bench_function("fast Sobel copy", move |b| {
        b.iter(|| { 
            fast_sobel_mut(black_box(&mut luma_image), 120); 
            for (gray_pixel, color_pixel) in luma_image.pixels().zip(fast_copy_color_image.pixels_mut()) {
    ...
    group.bench_function("Sobel copy", move |b| {
        b.iter(|| { 
            slower_sobel_mut(black_box(&mut luma_image_copy), 120); 
